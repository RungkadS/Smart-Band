...
TinyGPSPlus gps;
WidgetMap myMap(V8);  // Define the WidgetMap object
...
void loop()
{
  ...
  // Determine bpmCategory
  String bpmCategory;
  if (beatsPerMinute < 70) {
    bpmCategory = "Normal";
  } else if (beatsPerMinute >= 70 && beatsPerMinute <= 90) {
    bpmCategory = "Sedang";
  } else {
    bpmCategory = "Tinggi";
  }
  ...
  if (millis() - lastUpdate >= UPDATE_INTERVAL) {
    ...
    Blynk.virtualWrite(V3, stressLevel); // Kirim stress level ke Virtual Pin V3
    Blynk.virtualWrite(V4, emotionalResponse); // Kirim respon emosional ke Virtual Pin V4
    Blynk.virtualWrite(V5, sensorValue); // Kirim nilai GSR ke Virtual Pin V5
...
void updateMap() {
  ...
  if (gps.location.isValid()) {
    myMap.location(1, latitude, longitude, "Current Location");
  }
}
